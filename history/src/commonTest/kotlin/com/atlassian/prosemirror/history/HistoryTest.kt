...
class HistoryTest {
  ...
  fun command(state: PMEditorState, command: Command): PMEditorState {
    println("command $command state: ${state.doc}")
    var state = state
    command(state) { tr ->
      val state2 =state.apply(tr)
      state = state2
      println("command inside $command state: ${state.doc} state2: ${state2.doc}")
    }
    return state
  }
  ...
  fun `enables undo`() {
    ...
    var state = mkState()
    state = type(state, "a")
//    println("undo state a: ${state.doc.content}")
    state = type(state, "b")
    assertThat(state.doc).isEqualTo(doc { p { +"ab" } })
//    println("undo state before: ${state.doc}")
    state = command(state, undo)
//    println("undo state after: ${state.doc}")
...
//    println("test redo state: ${state.doc}")
//    assertThat(state.doc).isEqualTo(doc { p { +"ab" } })
//      println("redo end")
//  }
//
